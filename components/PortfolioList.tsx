...
export default function PortfolioList({ projects }: { projects: Project[] }) {
  ...
  const categories = ["All", ...new Set(projects.map((p) => p.category?.title || "Others"))];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredProjects.map((project) => (
          ...
          >
            <div className="relative aspect-4/3 overflow-hidden rounded-xl bg-gray-900 mb-4 border border-white/5">
              {project.mainImage && (
                <Image
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 flex items-center gap-2 text-white font-semibold">
